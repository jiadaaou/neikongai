...
from app.services.chunking_service_standards import chunking_service
...
class DocumentProcessor:
    ...
    async def process_document(self, document_id: int, file_path: str) -> Dict:
        ...
        try:
            ...
            # standards 专用：向量内容补充法律名称 + 条号，提高检索准确率
            conn = get_db_connection()
            cursor = conn.cursor()
            try:
                cursor.execute("SELECT title FROM legal_documents WHERE id = %s", (document_id,))
                row = cursor.fetchone()
                document_title = row[0] if row and row[0] else ""
            finally:
                cursor.close()
                conn.close()
            ...
            chunk_texts = []
            for c in chunks:
                article_no = c.get('article_start') or ''
                base_text = c.get('chunk_text') or ''
                if document_title and article_no:
                    vector_text = f"{document_title}\n{article_no}\n{base_text}"
                elif document_title:
                    vector_text = f"{document_title}\n{base_text}"
                else:
                    vector_text = base_text
                chunk_texts.append(vector_text)
