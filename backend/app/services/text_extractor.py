...
class TextExtractor:
    ...
    def extract_from_word(self, file_path: str) -> Dict:
        ...
        try:
            ...
            result['text'] = '\n'.join(content_parts)
            result['metadata']['total_parts'] = len(content_parts)
            result['metadata']['total_tables'] = len(doc.tables)
            
            # 模拟页面（每10个内容块算一页）
            for i in range(0, len(content_parts), 10):
                page_parts = content_parts[i:i+10]
                result['pages'].append({
                    'page_num': i // 10 + 1,
                    'text': '\n'.join(page_parts)
                })
            
            print(f"✅ Word 提取成功: {len(content_parts)} 个内容块, {len(doc.tables)} 个表格, {len(result['text'])} 字符")
